import { settleUserFacingContextAction, contextModeWord } from '../contextModePolicy.js';
...
export async function setContextMode(mode, {
    ...
    const requestIsCurrent = () => !this.destroyed && !signal?.aborted
...
export function clearSelectedLayers() {
    if (this.destroyed) return Promise.resolve({ targetIds: [], items: [], clearedIds: [], notClearedIds: [], cancelled: true });
    ...
    const operation = managerOperation.then((result) => {
      if (result.targetIds.length === 0) {
        ...
        this.showToast(`${result.notClearedIds.length} data layer${result.notClearedIds.length === 1 ? '' : 's'} could not be cleared`);
      } else {
        this.showToast(`Cleared ${result.clearedIds.length} data layer${result.clearedIds.length === 1 ? '' : 's'}`);
      }
      return result;
    }).catch((error) => {
