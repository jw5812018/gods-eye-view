...
export function _initGlobalContextPanel() {
    ...
    contextTabs.forEach((tab, index) => this.listen(tab, 'keydown', (event) => {
      let nextIndex = null;
    ...
    this.listen(this._globalContextFlightsBtn, 'click', () => {
      if (this.destroyed || this._contextModeChanging || this._clearSelectedLayersPromise) return;
      ...
      void this._runUserFacingContextAction(
        ...
        if (!this.destroyed && nextMode && shouldExpandGlobalContextPanel({
          action: 'contacts',
          explicitUserAction: true,
          succeeded: succeeded === true,
        })) this.actions.setPanelCollapsed('global-context-panel', false, { explicit: true });
      });
    });
    ...
    this.listen(this._installationsSearchBtn, 'click', () => {
      if (this.destroyed || !this._dataManager?.layers?.has('military-installations')) return;
      ...
      void this._runUserFacingContextAction(async (notificationToken) => {
        ...
        if (this.destroyed || enabled === false || !this._dataManager.isEnabled('military-installations')) return false;
        ...
        this.showToast(stats?.statusMessage || (stats?.status === 'zoom-in'
